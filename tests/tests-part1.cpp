...
TEST_CASE("Image scale scales pixel data in a reasonable length", "[weight=1][part=1]") {
  Image img = createRainbowImage();
  //360,100
  Image result = createRainbowImage();
  result.scale(720,700);

  REQUIRE( result.width()== 720);
  REQUIRE( result.height() ==200);
}

TEST_CASE("Image scale scales pixel data in a reasonable with(small)", "[weight=1][part=1]") {
  Image img = createRainbowImage();
  //360,100
  Image result = createRainbowImage();
  result.scale(720,70);
  REQUIRE( result.width() ==252);
  REQUIRE( result.height()== 70);
